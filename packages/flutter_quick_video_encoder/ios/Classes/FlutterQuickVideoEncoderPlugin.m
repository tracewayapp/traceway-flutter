...
- (void)handleMethodCall:(FlutterMethodCall *)call result:(FlutterResult)result
{
    @try
    {
        ...
        else if ([@"encodeFrames" isEqualToString:call.method])
        {
            NSDictionary *args = (NSDictionary *)call.arguments;
            NSArray<FlutterStandardTypedData *> *pngFrames = args[@"pngFrames"];
            int encWidth = [args[@"width"] intValue];
            int encHeight = [args[@"height"] intValue];
            int encFps = [args[@"fps"] intValue];
            int encBitrate = [args[@"videoBitrate"] intValue];
            NSString *profileLevelStr = args[@"profileLevel"];
            NSArray *touchPositions = args[@"touchPositions"];

            dispatch_async(dispatch_get_global_queue(QOS_CLASS_USER_INITIATED, 0), ^{
                @try {
                    NSData *mp4Data = [self doEncodeFrames:pngFrames
                                                    width:encWidth
                                                   height:encHeight
                                                      fps:encFps
                                                  bitrate:encBitrate
                                             profileLevel:profileLevelStr
                                           touchPositions:touchPositions];
                    dispatch_async(dispatch_get_main_queue(), ^{
                        result([FlutterStandardTypedData typedDataWithBytes:mp4Data]);
                    });
                }
                @catch (NSException *e) {
                    NSString *stackTrace = [[e callStackSymbols] componentsJoinedByString:@"\n"];
                    dispatch_async(dispatch_get_main_queue(), ^{
                        result([FlutterError errorWithCode:@"encodeFramesError"
                                                  message:[e reason]
                                                  details:stackTrace]);
                    });
                }
...
- (NSData *)doEncodeFrames:(NSArray<FlutterStandardTypedData *> *)pngFrames
                     width:(int)encWidth
                    height:(int)encHeight
                       fps:(int)encFps
                   bitrate:(int)encBitrate
              profileLevel:(NSString *)profileLevelStr
            touchPositions:(NSArray *)touchPositions
{
    // Temp file
    NSString *tempDir = NSTemporaryDirectory();
    NSString *filepath = [tempDir stringByAppendingPathComponent:
        [NSString stringWithFormat:@"fqve_batch_%@.mp4", [[NSUUID UUID] UUIDString]]];
    NSURL *fileURL = [NSURL fileURLWithPath:filepath];
    ...
    // Setup AVAssetWriter
    NSError *error = nil;
    AVAssetWriter *writer = [[AVAssetWriter alloc] initWithURL:fileURL
                                                      fileType:AVFileTypeQuickTimeMovie
                                                         error:&error];
    if (error) {
        @throw [NSException exceptionWithName:@"AVAssetWriterError"
    ...
    // Video compression settings
    NSMutableDictionary *compressionProperties = [NSMutableDictionary dictionaryWithDictionary:@{
        AVVideoAverageBitRateKey : @(encBitrate)
    }];
    if (![profileLevelStr isEqualToString:@"any"]) {
        [compressionProperties setObject:[self parseProfileLevel:profileLevelStr]
    ...
    // Encode each frame
    for (int i = 0; i < (int)pngFrames.count; i++) {
        NSData *pngData = pngFrames[i].data;
        ...
        CGImageRef cgRef = [nsImage CGImageForProposedRect:NULL context:nil hints:nil];
        // We'll use cgRef directly below
#endif

#if TARGET_OS_IPHONE
        CGImageRef cgImage = image.CGImage;
#else
        CGImageRef cgImage = cgRef;
#endif

        if (!cgImage) {
            @throw [NSException exceptionWithName:@"PNGDecodeError"
                                           reason:[NSString stringWithFormat:@"Failed to decode PNG frame %d", i]
                                         userInfo:nil];
        }
        ...
        CVReturn cvReturn = CVPixelBufferCreate(
            kCFAllocatorDefault, encWidth, encHeight,
            kCVPixelFormatType_32BGRA,
            (__bridge CFDictionaryRef)attributes,
            &pixelBuffer);

        if (cvReturn != kCVReturnSuccess) {
            @throw [NSException exceptionWithName:@"PixelBufferError"
        ...
        CGContextRef context = CGBitmapContextCreate(
            baseAddress, encWidth, encHeight, 8, bytesPerRow,
            colorSpace, kCGBitmapByteOrder32Little | kCGImageAlphaPremultipliedFirst);

        // Draw the image (flipped vertically for Core Graphics coordinate system)
        CGContextDrawImage(context, CGRectMake(0, 0, encWidth, encHeight), cgImage);

        // Draw touch circle if position provided
        if (touchPositions && i < (int)touchPositions.count &&
            touchPositions[i] != [NSNull null] && touchPositions[i] != nil) {
            NSDictionary *pos = touchPositions[i];
            CGFloat cx = [pos[@"x"] doubleValue];
            CGFloat cy = [pos[@"y"] doubleValue];
            // Core Graphics has origin at bottom-left, flip Y
            CGFloat flippedY = encHeight - cy;
            CGFloat radius = 18.0;

            // Border ring (opaque)
            CGContextSetRGBFillColor(context, 75.0/255.0, 163.0/255.0, 247.0/255.0, 200.0/255.0);
            CGContextFillEllipseInRect(context,
                CGRectMake(cx - radius, flippedY - radius, radius * 2, radius * 2));
            // Inner fill (semi-transparent)
            CGFloat inner = radius - 3.0;
            CGContextSetRGBFillColor(context, 75.0/255.0, 163.0/255.0, 247.0/255.0, 80.0/255.0);
            CGContextFillEllipseInRect(context,
                CGRectMake(cx - inner, flippedY - inner, inner * 2, inner * 2));
        }
        ...
        timingInfo.duration = CMTimeMake(1, encFps);
        timingInfo.decodeTimeStamp = kCMTimeInvalid;
        timingInfo.presentationTimeStamp = CMTimeMake(i, encFps);
        ...
        // Wait for input to be ready (simple sleep-based wait for background thread)
    ...
    if (writer.status == AVAssetWriterStatusFailed) {
        @throw [NSException exceptionWithName:@"AVAssetWriterFinishError"
                                       reason:[writer.error localizedDescription]
                                     userInfo:nil];
    }

    // Read MP4 bytes
    NSData *mp4Data = [NSData dataWithContentsOfFile:filepath];
