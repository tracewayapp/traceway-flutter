    ...
    MethodChannel.MethodCallHandler
{
    ...
    public void onMethodCall(MethodCall call, MethodChannel.Result result) {
        try{
            switch (call.method) {
                ...
                case "encodeFrames":
                {
                    List<byte[]> pngFrames = call.argument("pngFrames");
                    int encWidth = call.argument("width");
                    int encHeight = call.argument("height");
                    int encFps = call.argument("fps");
                    int encBitrate = call.argument("videoBitrate");
                    List<Map<String, Double>> touchPositions = call.argument("touchPositions");
                    ...
                    executor.execute(() -> {
                        try {
                            byte[] mp4Bytes = doEncodeFrames(
                                pngFrames, encWidth, encHeight, encFps, encBitrate, touchPositions);
                            mainHandler.post(() -> result.success(mp4Bytes));
                        } catch (Exception e) {
                            ...
                            mainHandler.post(() ->
                                result.error("encodeFramesError", e.toString(), stackTrace));
                        } finally {
                            executor.shutdown();
                        }
                    });
    ...
    /**
     * Encodes a list of PNG frames into an MP4 file entirely on the calling thread.
     * Meant to be called from a background executor.
     */
    private byte[] doEncodeFrames(
            List<byte[]> pngFrames,
            int width, int height, int fps, int videoBitrate,
            List<Map<String, Double>> touchPositions) throws Exception {

        // Temp file
        File tempFile = File.createTempFile("fqve_batch_", ".mp4");
        String filepath = tempFile.getAbsolutePath();

        // Setup encoder (reuse same logic as 'setup')
        int colorFormat = getColorFormat();
        MediaFormat videoFormat = MediaFormat.createVideoFormat("video/avc", width, height);
        videoFormat.setInteger(MediaFormat.KEY_BIT_RATE, videoBitrate);
        videoFormat.setInteger(MediaFormat.KEY_FRAME_RATE, fps);
        ...
        int frameIdx = 0;
        Queue<EncodedData> outQueue = new LinkedList<>();

        // Encode each frame
        for (int i = 0; i < pngFrames.size(); i++) {
            byte[] pngBytes = pngFrames.get(i);

            // Decode PNG to Bitmap
            Bitmap bitmap = BitmapFactory.decodeByteArray(pngBytes, 0, pngBytes.length);
            if (bitmap == null) {
                throw new RuntimeException("Failed to decode PNG frame " + i);
            }
            ...
            for (int p = 0; p < pixels.length; p++) {
                int color = pixels[p];
                rgba[p * 4]     = (byte) ((color >> 16) & 0xFF); // R
                rgba[p * 4 + 1] = (byte) ((color >> 8) & 0xFF);  // G
                rgba[p * 4 + 2] = (byte) (color & 0xFF);         // B
                rgba[p * 4 + 3] = (byte) ((color >> 24) & 0xFF); // A
            }

            // Draw touch circle if position provided
            if (touchPositions != null && i < touchPositions.size() && touchPositions.get(i) != null) {
                Map<String, Double> pos = touchPositions.get(i);
                int cx = pos.get("x").intValue();
                int cy = pos.get("y").intValue();
                drawTouchCircle(rgba, width, height, cx, cy);
            }

            // Convert to YUV420
            byte[] yuv420 = rgbaToYuv420Planar(rgba, width, height);

            // Feed encoder
            long presentationTime = frameIdx * 1000000L / fps;
            int inIdx = encoder.dequeueInputBuffer(-1);
            if (inIdx >= 0) {
                ByteBuffer buffer = encoder.getInputBuffer(inIdx);
                Image image = encoder.getInputImage(inIdx);
                fillImage(image, yuv420, width, height);
                encoder.queueInputBuffer(inIdx, 0, buffer.capacity(), presentationTime, 0);
            }
            frameIdx++;
            ...
            while (true) {
                ...
                if (status == MediaCodec.INFO_TRY_AGAIN_LATER) {
                    ...
                    while (!outQueue.isEmpty()) {
                        EncodedData d = outQueue.poll();
                        d.byteBuffer.position(d.bufferInfo.offset);
                        d.byteBuffer.limit(d.bufferInfo.offset + d.bufferInfo.size);
                        muxer.writeSampleData(videoTrackIndex, d.byteBuffer, d.bufferInfo);
                    }
                } else if (status >= 0) {
                    ...
                    if (bufferInfo.size != 0) {
                        if (muxerStarted) {
                            ...
                            outQueue.add(new EncodedData(encodedData, bufferInfo));
                        }
                    }
        ...
        int eosIdx = encoder.dequeueInputBuffer(-1);
        if (eosIdx >= 0) {
        ...
        MediaCodec.BufferInfo bufferInfo = new MediaCodec.BufferInfo();
        while (true) {
            int status = encoder.dequeueOutputBuffer(bufferInfo, 10000);
            if (status == MediaCodec.INFO_TRY_AGAIN_LATER) {
                continue;
            } else if (status == MediaCodec.INFO_OUTPUT_FORMAT_CHANGED) {
                if (!muxerStarted) {
        ...
        byte[] mp4Bytes = new byte[(int) raf.length()];
        raf.readFully(mp4Bytes);
        raf.close();
        tempFile.delete();
    ...
    private void drawTouchCircle(byte[] rgba, int width, int height, int cx, int cy) {
        final int radius = 18;
        final int borderWidth = 3;
        final int innerRadius = radius - borderWidth;

        // #4ba3f7
        final int circleR = 75, circleG = 163, circleB = 247;
        ...
        for (int y = minY; y <= maxY; y++) {
            for (int x = minX; x <= maxX; x++) {
                ...
                float alpha = distSq > innerRadius * innerRadius ? 200f / 255f : 80f / 255f;
                float invAlpha = 1f - alpha;

                rgba[offset]     = (byte) Math.round(circleR * alpha + (rgba[offset] & 0xFF) * invAlpha);
                rgba[offset + 1] = (byte) Math.round(circleG * alpha + (rgba[offset + 1] & 0xFF) * invAlpha);
                rgba[offset + 2] = (byte) Math.round(circleB * alpha + (rgba[offset + 2] & 0xFF) * invAlpha);
            }
        }
    }
